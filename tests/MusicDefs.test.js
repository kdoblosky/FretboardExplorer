...
describe("Testing Tunings", () => {
  test("Tunings contains Standard", () => {
    expect(MusicDefs.Tunings.find((f) => f.Id === "Standard").Name).toBe("Standard Tuning");
  });
  test("Standard Tuning contains correct sequence of Strings", () => {
    expect(MusicDefs.Tunings.find((f) => f.Id === "Standard").Strings).toStrictEqual(["E", "A", "D", "G", "B", "E"]);
  });
});

describe("Testing ScaleTypes", () => {
  test("ScaleTypes contains Minor", () => {
    expect(MusicDefs.ScaleTypes.find((f) => f.Id === "Minor").Name).toBe("Minor");
  });
  test("Dorian ScaleType correct sequence of Intervals", () => {
    expect(MusicDefs.ScaleTypes.find((f) => f.Id === "Dorian").Intervals).toStrictEqual([
      Intervals.WholeStep,
      Intervals.HalfStep,
      Intervals.WholeStep,
      Intervals.WholeStep,
...
describe("Testing ChordTypes", () => {
  ...
  test("MinorSeventh ChordType correct sequence of Intervals", () => {
    expect(MusicDefs.ChordTypes.find((f) => f.Name === "MinorSeventh").Intervals).toStrictEqual([
