...
describe("Test Fretboard with Standard tuning", () => {
  ...
  test("Test Fretboard.GetFretsWithAttribute", () => {
    expect(Fretboard.GetFretsWithAttribute(FretAttribute.ScaleSecond).length).toBe(7);
    expect(Fretboard.GetFretsWithAttribute(FretAttribute.ScaleThird).length).toBe(6);
  });
});
...
describe("Test Fretboard with Capo", () => {
  beforeAll(() => {
    var tuningS = MusicDefs.GetTuning("Standard");
    Fretboard.SetTuning(tuningS, 3);
  });

  test("Test First String has correct note at open fret", () => {
    expect(Fretboard.FretboardMap[0].Frets.find((f) => f.FretNumber === 0).Note.Name).toBe("G");
  });
});
